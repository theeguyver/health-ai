...
const SessionList = ({ sessions, onSelectSession, onRemoveSession }) => {
  return (
    <div className="fixed left-0 top-1/2 transform -translate-y-1/2 w-72 h-full bg-blue-900 bg-opacity-30 text-white p-4 overflow-y-auto">
      <div className="empty-div" style={{ height: '70px' }}></div> {/* Highlighted modification */}
      <div className="font-bold text-xl mb-4">Session History</div>
...
const ChatbotPage = () => {
  ...
  const handleSelectSession = (session) => {
    ...
    // User should not be able to enter data to past sessions. 
    setShowOverlay(true);
  };
