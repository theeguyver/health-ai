...
app.post('/addChat', async (req, res) => {
    ...
    try {
        ...
        if (userSession) {
            ...
            await db.collection(collectionName).updateOne({ userId }, { $push: { "session.$[session].sessiondata": { $each: messages } } }, { arrayFilters: [{ "session.sessionid": userSession.session[0].sessionid }] });
        } else {
